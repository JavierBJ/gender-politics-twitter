...
rel = predict_gender.RelevanceByLassoRegression(tweets, labels, extractor=feature_extraction.BinaryBOW(lambda x: x.get_form(), keep_words_rank=3000, remove_stopwords=True))
print('\t',len(rel.features_idx),'features extracted.')
mis = rel.compute()
print('\tRelevances computed.')
rel.show(top=100, to=open('out_relevance/author_lasso_form_rank3000_sw.txt', 'w'))
...
rel = predict_gender.RelevanceByLassoRegression(tweets, labels, extractor=feature_extraction.BinaryBOW(lambda x: x.get_form(), keep_words_rank=5000, remove_stopwords=True))
print('\t',len(rel.features_idx),'features extracted.')
mis = rel.compute()
print('\tRelevances computed.')
rel.show(top=100, to=open('out_relevance/receiver_lasso_form_rank5000_sw.txt', 'w'))
